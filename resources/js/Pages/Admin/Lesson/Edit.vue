...
// Initialize CKEditor once the component mounts
onMounted(() => {
...
const submitForm = () => {
  ...
  form.put(route('lessons.update', props.lesson.id), {
    preserveScroll: true,
    onSuccess: () => {
      // Let Inertia handle the redirect back to the index page so the
      // flash/session data from the controller is preserved.
    },
    onError: () => {
      // Scroll to top so any validation errors are visible
            ...
            <div class="bg-white shadow-sm sm:rounded-lg p-8 border border-gray-200">
                <form @submit.prevent="submitForm" class="space-y-6">
                    <div class="grid grid-cols-1 gap-8 md:grid-cols-2">
                        <!-- Title Field Input -->
                        <div>
                            <label for="title" class="block text-base font-semibold text-gray-900 mb-3">Lesson Title</label>
                            <input
                                type="text"
                                name="title"
                                id="title"
                        ...
                        <div>
                            <label for="course_id" class="block text-base font-semibold text-gray-900 mb-3">Course Assign</label>
                            <select
                                id="course_id"
                            ...
                            <span v-if="form.errors.course_id" class="text-red-700 text-sm block mt-2">{{ form.errors.course_id }}</span>
                        </div>

                        <!-- Lesson Status Selection Dropdown -->
                        <div>
                            <label for="status" class="block text-base font-semibold text-gray-900 mb-3">Status</label>
                            <select
                                id="status"
                                v-model="form.status"
                                class="w-full px-4 py-2.5 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                        ...
                        <div>
                            <label for="type" class="block text-base font-semibold text-gray-900 mb-3">Type</label>
                            <select
                                id="type"
                                name="type"
                                v-model="form.type"
                            ...
                            <span v-if="form.errors.type" class="text-red-700 text-sm block mt-2">{{ form.errors.type }}</span>
                        </div>

                        <!-- Description Box (Full Width Spans) -->
                        <div class="md:col-span-2">
                            <label for="description" class="block text-base font-semibold text-gray-900 mb-3">Description</label>
                            <textarea
                                id="description"
                        ...
                        <div class="md:col-span-2">
                            <label for="content" class="block text-base font-semibold text-gray-900 mb-3">Lesson Content</label>
                            <textarea
                                id="content"
                            ...
                            ></textarea>
                            <span v-if="form.errors.content" class="text-red-700 text-sm block mt-2 font-medium">{{ form.errors.content }}</span>
                        </div>
                    </div>
