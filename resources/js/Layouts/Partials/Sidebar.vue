...
// Fetch user data details globally from the shared Inertia session state
const page = usePage();
const currentUser = computed(() => page.props.auth?.user || { role: '' });

// Build a path validation helper to replace Blade's request()->routeIs() utility
...
<template>
    ...
    <aside
        class="w-80 flex-shrink-0 min-h-screen bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 flex flex-col z-20">
        ...
        <div class="flex-1">
            ...
            <nav v-if="currentUser.role === 'Admin' || currentUser.role === 'Teacher'" class="py-4 px-3 space-y-2">
                ...
                <Link v-if="currentUser.role === 'Admin'" href="/trainingrecord"
                    :class="[isCurrentRoute('/trainingrecord') ? 'bg-indigo-50 text-indigo-700 dark:bg-gray-700 dark:text-indigo-400' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700/50 hover:text-gray-900']"
                    class="flex items-center px-4 py-2.5 text-sm font-medium rounded-lg transition-colors group">
                    <svg :class="[isCurrentRoute('/trainingrecord') ? 'text-indigo-600 dark:text-indigo-400' : 'text-gray-400 group-hover:text-gray-500']"
            ...
            <nav v-if="currentUser.role === 'Learner'" class="py-4 px-3 space-y-2">
                ...
                <Link href="/dashboard"
                    :class="[isCurrentRoute('/dashboard') ? 'bg-indigo-50 text-indigo-700 dark:bg-gray-700 dark:text-indigo-400' : 'text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-700/50 hover:text-gray-900']"
                    class="flex items-center px-4 py-2.5 text-sm font-medium rounded-lg transition-colors group">
                    <svg :class="[isCurrentRoute('/dashboard') ? 'text-indigo-600 dark:text-indigo-400' : 'text-gray-400 group-hover:text-gray-500']"
                        class="w-5 h-5 mr-3" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                        stroke-width="1.5" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round"
                            d="M4.26 10.147L12 15l7.74-4.853a1 1 0 000-1.707L12 3.586 4.26 8.44a1 1 0 000 1.707z" />
                        <path stroke-linecap="round" stroke-linejoin="round"
                            d="M12 15v6M12 21a2.121 2.121 0 01-3-3m3 3a2.121 2.121 0 003-3m-6 0h6" />
                    </svg>
                    MY LEARNING
                </Link>

                <!-- Training Records Student (LEGACY BLADE VIEW: Swapped out for plain HTML anchor tag) -->
                <Link href="/trainingrecords"
